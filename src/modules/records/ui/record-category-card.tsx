...
import type {
  EntryPresence,
  RecordEditDraft,
} from "@/modules/records/application/get-record-hub";
import type { RecordType } from "@/modules/records/domain/types";
import {
  clearRecordDraft,
  createIdempotencyKey,
  writeRecordDraft,
} from "./record-form-shell";
...
const draftPathnameFor = (href: string): string =>
  href.split("?", 1)[0] || href;
...
export const RecordCategoryCard = ({
  ...
  const deleteConfirmation =
    records.length > 1
      ? `${category} 기록 ${records.length}건을 모두 삭제할까요?`
      : `${category} 기록을 삭제할까요?`;
  const [deleteState, deleteAction, deleting] = useActionState(
    async (
      _previous: null | Awaited<ReturnType<typeof deleteRecordBatchAction>>,
      formData: FormData,
    ) => {
    ...
    >
      ...
      <div className={styles.categorySummary}>
        <span className={styles.categoryIcon} aria-hidden="true">
          {category.includes("휴대폰") ? "수" : category.slice(0, 1)}
        </span>
        <div>
          <h3>{category}</h3>
      ...
      <div className={styles.categoryFooter}>
        <span className={styles.statusPill} data-status={presence}>
          {statusLabel}
        </span>
        <div className={styles.categoryActions}>
          <Link
            ...
            href={createHref}
            onClick={() => clearRecordDraft(draftPathname, draftScope)}
          >
            추가
          </Link>
          {presence !== "empty" ? (
            <Link
              ...
              onClick={() => {
                if (editDraft)
                  writeRecordDraft(draftPathname, editDraft, draftScope);
              }}
            >
              수정
            </Link>
          ...
          {presence !== "empty" ? (
            <form
              ...
              action={deleteAction}
              onSubmit={(event) => {
                if (!window.confirm(deleteConfirmation)) event.preventDefault();
              }}
            >
              <input
                type="hidden"
