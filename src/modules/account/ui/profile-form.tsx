...
export const ProfileForm = ({ identity, profile, idPrefix = "profile" }: Readonly<{ identity: { email: string | null }; profile: { nickname: string; timezone: string }; idPrefix?: string }>) => {
  ...
  return <form action={formAction} aria-busy={pending} className={styles.form}>
    <label htmlFor={`${idPrefix}-email`}>로그인 이메일</label><input aria-label="로그인 이메일" id={`${idPrefix}-email`} readOnly value={identity.email ?? "인증된 이메일 정보를 불러올 수 없어요."} />
    <p className={styles.privacyNote}>이메일 변경은 인증 제공자에서만 관리합니다.</p>
    <label htmlFor={`${idPrefix}-nickname`}>닉네임</label><input aria-describedby={nicknameError ? nicknameErrorId : undefined} aria-invalid={nicknameError ? true : undefined} defaultValue={state.values.nickname ?? profile.nickname} id={`${idPrefix}-nickname`} maxLength={40} name="nickname" required />
    {nicknameError ? <p className={styles.error} id={nicknameErrorId}>{nicknameError}</p> : null}
    <label htmlFor={`${idPrefix}-timezone`}>타임존</label><select defaultValue={state.values.timezone ?? profile.timezone} id={`${idPrefix}-timezone`} name="timezone" required>{timezones.map((timezone) => <option key={timezone} value={timezone}>{timezone}</option>)}</select>
