...
export const SleepGuide = ({ localDate }: { localDate: string }) => {
  ...
  useEffect(() => { if (!running) return; const timer = window.setInterval(() => setElapsed((value) => Math.min(300, value + 1)), 1000); return () => window.clearInterval(timer); }, [running]);
