...
test("shows distinct record cards and routes the quick-record CTA to a live form", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("link", { name: "빠른 기록 시작" })).toHaveAttribute("href", "/record/caffeine?step=brand&mode=create");
  ...
  const expectedRoutes = [
    ...
    ["운동", "/record/meal-health?step=exercise-and-wellness&focus=exercise&mode=create"],
...
test("starts a fresh record from the quick CTA even when an abandoned edit draft exists", async ({ page }, testInfo) => {
  await setupE2eUser(page.request, testInfo);
  await page.goto("/record");
  await page.evaluate(() => {
    window.sessionStorage.setItem("record-draft:/record/caffeine", JSON.stringify({
      schemaVersion: 1,
      draft: {
        step: "confirm",
...
test("keeps an unfinished create draft and idempotency key through a fresh-create reload", async ({ page }, testInfo) => {
  await setupE2eUser(page.request, testInfo);
  ...
  await expect(page).toHaveURL(/\/record\/caffeine\?step=brand&mode=create$/);
  await expect(page.getByLabel("브랜드", { exact: true })).toHaveValue("재시도 카페");
  await expect(page.locator('input[name="idempotencyKey"]')).toHaveValue(idempotencyKey);
  ...
  await expect.poll(async () => JSON.parse(await page.locator('input[name="items"]').inputValue())[0]?.recordId).toBeUndefined();
...
test("persists an alcohol can unit, restores it for editing, and deletes the record", async ({ page }, testInfo) => {
  await setupE2eUser(page.request, testInfo);
  await page.goto("/record");

  const alcoholCard = page.locator("article").filter({ has: page.getByRole("heading", { name: "알코올" }) });
  ...
  await page.getByLabel("마신 시각", { exact: true }).fill("2026-08-20T20:00");
  ...
  await page.getByRole("button", { name: "양 입력하기" }).click();
  await expect(page.getByLabel("마신 양 기준", { exact: true })).toHaveValue("can");
  await page.getByLabel("마신 양(기록 단위)", { exact: true }).fill("1.5");
  await page.getByRole("button", { name: "기록 확인" }).click();
  await page.getByRole("button", { name: "음주 저장" }).click();

  await expect(page).toHaveURL(/\/record$/);
  const updatedAlcoholCard = page.locator("article").filter({ has: page.getByRole("heading", { name: "알코올" }) });
  page.once("dialog", (dialog) => dialog.accept());
  await updatedAlcoholCard.getByRole("button", { name: "삭제" }).click();
  await expect(updatedAlcoholCard.getByText("미입력", { exact: true })).toBeVisible();
});

test("persists exercise and wellness records and appends a second exercise instead of replacing the first", async ({ page }, testInfo) => {
  await setupE2eUser(page.request, testInfo);

  const openNewExerciseAndWellness = async () => {
    await page.goto("/record");
    const card = page.locator("article").filter({ has: page.getByRole("heading", { name: "운동" }) });
    await card.getByRole("link", { name: "추가" }).click();
    await expect(page).toHaveURL(/\/record\/meal-health\?step=exercise-and-wellness&focus=exercise&mode=create$/);
  };
  const saveExerciseAndWellness = async (exerciseType: string, startedAt: string, endedAt: string) => {
    ...
    await page.getByLabel("컨디션 날짜", { exact: true }).fill("2026-08-20");
    await page.getByRole("button", { name: "기록 확인" }).click();
  ...
  await saveExerciseAndWellness("조깅", "2026-08-20T09:00", "2026-08-20T09:30");

  const firstExerciseCard = page.locator("article").filter({ has: page.getByRole("heading", { name: "운동" }) });
  await expect(firstExerciseCard.getByRole("link", { name: "수정" }))
    .toHaveAttribute("href", "/record/meal-health?step=exercise-and-wellness&focus=exercise");
  await firstExerciseCard.getByRole("link", { name: "수정" }).click();
  await expect(page).toHaveURL(/\/record\/meal-health\?step=exercise-and-wellness&focus=exercise$/);
  await expect.poll(() => page.evaluate(() => window.sessionStorage.getItem("record-draft:/record/meal-health:exercise")))
  ...
  await saveExerciseAndWellness("요가", "2026-08-20T18:00", "2026-08-20T18:30");

  const exerciseCard = page.locator("article").filter({ has: page.getByRole("heading", { name: "운동" }) });
  ...
  await expect(exerciseCard.getByText("완료", { exact: true })).toBeVisible();
  await expect(exerciseCard).toContainText("저장 1건");
});
